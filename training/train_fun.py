...
from utils.misc import get_logger, set_initial_seed
...
def __train_loop__(dataset, model, optimiser, split_idx, n_epochs, patience, txt_logger,
    ...
    val_mask = dataset.val_mask[:, split_idx]
    test_mask = dataset.test_mask[:, split_idx]
...
def end_to_end_training(config, split_idx, exp_dir, write_on_console):
    seed = config.training_config.seed if 'seed' in config.training_config else -1
